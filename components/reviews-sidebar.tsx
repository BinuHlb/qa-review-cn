...
function getQuickFiltersForPage(pathname: string, filters?: ReviewsSidebarProps['filters']) {
  if (pathname.includes('/admin/reviews') || pathname.includes('/admin/final-reviews')) {
    return [
      ...
      { icon: Clock, label: "Overdue", action: () => filters?.onStatusChange && filters.onStatusChange('Overdue') },
      { icon: CheckCircle, label: "In Progress", action: () => filters?.onStatusChange && filters.onStatusChange('In Progress') },
      { icon: Star, label: "Completed", action: () => filters?.onStatusChange && filters.onStatusChange('Completed') },
    ]
  }
  
  if (pathname.includes('/admin/reviewers')) {
    return [
      { icon: CheckCircle, label: "Active Reviewers", action: () => filters?.onStatusChange && filters.onStatusChange('Active') },
      { icon: Clock, label: "Pending Reviewers", action: () => filters?.onStatusChange && filters.onStatusChange('Pending') },
      { icon: Users, label: "Senior Reviewers", action: () => filters?.onGradeChange && filters.onGradeChange('Senior') },
    ]
  }
  
  if (pathname.includes('/admin/member-firms')) {
    return [
      { icon: CheckCircle, label: "Active Firms", action: () => filters?.onStatusChange && filters.onStatusChange('Active') },
      { icon: AlertCircle, label: "High Risk", action: () => filters?.onPriorityChange && filters.onPriorityChange('High') },
      { icon: Building2, label: "Large Firms", action: () => filters?.onGradeChange && filters.onGradeChange('Large') },
    ]
  }
...
export function ReviewsSidebar({ 
  ...
  className = "",
  ...props 
}: ReviewsSidebarProps & React.ComponentProps<typeof Sidebar>) {
  const pathname = usePathname()
  const quickActions = getQuickActionsForPage(pathname)
  const quickFilters = getQuickFiltersForPage(pathname, filters)
  return (
              ...
              <SidebarMenu>
                ...
                <SidebarMenuItem>
                  <div className="px-2 py-1">
                    ...
                    <Select value={filters.gradeFilter} onValueChange={filters.onGradeChange}>
                      <SelectTrigger className="h-7 text-xs bg-sidebar-accent/20 border-sidebar-border">
                        <SelectValue placeholder="All" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all" className="text-xs">All Grades</SelectItem>
                        {filters.gradeOptions.map((grade) => (
                ...
                <SidebarMenuItem>
                  <div className="px-2 py-1">
                    ...
                    <Select value={filters.priorityFilter} onValueChange={filters.onPriorityChange}>
                      <SelectTrigger className="h-7 text-xs bg-sidebar-accent/20 border-sidebar-border">
                        <SelectValue placeholder="All" />
                      </SelectTrigger>
                      <SelectContent>
                ...
                <SidebarMenuItem>
                  <div className="px-2 py-1">
                    ...
                    <Select value={filters.countryFilter} onValueChange={filters.onCountryChange}>
                      <SelectTrigger className="h-7 text-xs bg-sidebar-accent/20 border-sidebar-border">
                        <SelectValue placeholder="All" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all" className="text-xs">All Regions</SelectItem>
                        {filters.countryOptions.map((country) => (
                  ...
                  <div className="px-2 py-1">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => filters.onViewModeChange("card")}
                        className="flex-1 h-7 text-xs px-2"
                      >
